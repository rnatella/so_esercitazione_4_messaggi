...
if(/Registro:\sInvio\smessaggio\sdi\srisposta\s\(id_server=(\d+),\sid_coda=(\d+)\)/) { if($server{$1}!=$2) { exit(1); } }
...
END { if(! exists $exit{1} || ! exists $exit{2}) { exit(1); }  if($#requests!=$#received) { exit(1); }  @s1=sort @requests; @s2=sort @received; for $i(0..$#s1) { if($s1[$i]!=$s2[$i]) { exit(1); } } if($total<9) { exit(1); } }
' $OUTPUT
